...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['accion']) && $_POST['accion'] == 'procesar_venta') {
    ...
    foreach ($productos as $p) {
        ...
        // Descontar stock
        mysqli_query($conn, "UPDATE productos SET stock = stock - $cantidad WHERE id = $id_prod");
        ...
        <div class="panel-productos">
            ...
            <div class="productos-grid" id="productosGrid">
                ...
                <button class="prod-btn" data-id="<?php echo $p['id']; ?>"
                        ...
                        onclick="agregarAlCarrito(<?php echo $p['id']; ?>, '<?php echo addslashes($p['nombre']); ?>', <?php echo $p['precio']; ?>, <?php echo $p['stock']; ?>)">
        ...
        <div class="panel-carrito">
            ...
            <div class="descuento-wrap">
                ...
                <div class="descuento-input">
                    <input type="number" id="descuento" min="0" max="100" value="0" placeholder="0" oninput="calcularTotales()">
                    <span>%</span>
                </div>
            </div>
            ...
            <div class="metodo-selector">
                <div class="metodo-opt">
                    <input type="radio" name="metodo" id="m-efectivo" value="efectivo" checked onchange="mostrarEfectivo()">
                    <label for="m-efectivo" class="lbl-efectivo">
                        <i class="fas fa-money-bill-wave"></i>
                        Efectivo
                    </label>
                </div>
                <div class="metodo-opt">
                    <input type="radio" name="metodo" id="m-debito" value="debito" onchange="mostrarEfectivo()">
                    <label for="m-debito" class="lbl-debito">
                        <i class="fas fa-credit-card"></i>
                        Débito
                    </label>
                </div>
                <div class="metodo-opt">
                    <input type="radio" name="metodo" id="m-credito" value="credito" onchange="mostrarEfectivo()">
                    <label for="m-credito" class="lbl-credito">
                        <i class="fas fa-credit-card"></i>
                        Crédito
                    </label>
                </div>
                <div class="metodo-opt">
                    <input type="radio" name="metodo" id="m-paypal" value="paypal" onchange="mostrarEfectivo()">
                    <label for="m-paypal" class="lbl-paypal">
    ...
    function renderCarrito() {
        ...
        carrito.forEach((item, idx) => {
            ...
            <div class="carrito-item">
                ...
                <div class="qty-ctrl">
                    <button class="qty-btn qty-minus" onclick="cambiarCantidad(${idx}, -1)"><i class="fas fa-minus"></i></button>
                    <span class="qty-num">${item.cantidad}</span>
                    <button class="qty-btn qty-plus"  onclick="cambiarCantidad(${idx}, +1)"><i class="fas fa-plus"></i></button>
                </div>
                <div class="ci-subtotal">$${(item.precio * item.cantidad).toFixed(2)}</div>
                <button onclick="quitarItem(${idx})" style="background:none;border:none;color:rgba(255,77,77,.5);cursor:pointer;padding:0 0 0 6px;font-size:.85rem;" onmouseover="this.style.color='var(--danger)'" onmouseout="this.style.color='rgba(255,77,77,.5)'">
        ...
        // Actualizar badges en los botones del catálogo
        document.querySelectorAll('.prod-btn').forEach(btn => {
            const id   = parseInt(btn.dataset.id);
            const item = carrito.find(i => parseInt(i.id) === id);
        ...
        fetch(`punto_venta.php?buscar=${encodeURIComponent(term)}`)
            ...
            .then(data => {
                ...
                box.innerHTML = data.map(p => `
                    <div class="search-result-item" onclick="agregarAlCarrito(${p.id}, '${p.nombre.replace(/'/g,"\\'")}', ${p.precio}, ${p.stock}); limpiarBusqueda();">
                        <div>
